...
links_XX.json (XX は数値) の JSONをインポートする際に、すでにWebストレージに登録されているデータを上書きするのか、マージするかを選択できる。
マージの場合は、重複するデータについて、"登録されているデータのみ残す"、"インポートするデータで上書きする"、"登録されているデータの "note" 要素に、インポートするデータの "note" 要素の情報を追記する" の3つを選択できる。 

なお、"Name" 要素と、"URL"要素 の2つの値が一致した場合、同じデータと判定する。


## ログについて

ユーザーの以下の操作履歴をログとして記録する。

・JSONを取込、出力したとき（ファイル名もログに記録）
・チェックボックスの設定を変更したとき（変更された値もログに記録）
・リンクをクリックしたとき（クリックしたリンクのNameをログに記録）
・検索をしたとき（検索キーもログに記録）
・リセットボタンをクリックしたとき
